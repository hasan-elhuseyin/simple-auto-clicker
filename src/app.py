...
from src import macro, macro_manager
from src.constants import RECORD_KEY, REPLAY_KEY
...
def run_app():
    ...
    record_btn = tk.Button(
        ...
        width=35,
        command=lambda: macro.start_recording(status_var.set) 
        if macro.mode=="idle" else macro.stop_recording(status_var.set)
    )
